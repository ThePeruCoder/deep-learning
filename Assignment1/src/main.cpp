...
vector<DataPoint> filter_relabel_tanh(const vector<DataPoint>& full_data,double target_class,double negative_class ){
    vector<DataPoint> filtered;
...
double calculate_accuracy_tanh(Perceptron& p,const vector<DataPoint>& test_data){
    int correct_guesses = 0;
...
void helper_tanh(const vector<DataPoint>& train,const vector<DataPoint>& test,double target,double negative,const string& match_name){
    cout << " Match: " << match_name << endl;

    vector<DataPoint> train_subset = filter_relabel_tanh(train,target,negative);
    vector<DataPoint> test_subset = filter_relabel_tanh(test,target,negative);

    int input_size = train[0].features.size();  
    Perceptron p(input_size,0.001,getTanh());
    ...
    double accuracy = calculate_accuracy_tanh(p, test_subset);
    cout << " Accuracy: " << accuracy << "%" << endl;
}
...
void run_ls_tanh(){
    ...
    helper_tanh(train_set, test_set, 0.0, 1.0, "Class 0 vs Class 1");
    helper_tanh(train_set, test_set, 0.0, 2.0, "Class 0 vs Class 2");
    helper_tanh(train_set, test_set, 1.0, 2.0, "Class 1 vs Class 2");

}


void run_nls_tanh(){
    cout << " Non Linearly Separable (One vs One) TanH" << endl;
